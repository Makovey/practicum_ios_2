...
struct NetworkClientStub: INetworkClient {
    let isNeededError: Bool
    ...
    func fetch(url: URL, handler: @escaping (Result<Data, MovieQuiz.NetworkError>) -> Void) {
        if isNeededError {
            handler(.failure(.noInternetConnectionError))
        } else {
